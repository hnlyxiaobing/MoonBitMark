...
| P1 | MCP logger utility | `src/mcp/util/logger.mbt` still uses a fixed timestamp and writes to stdout if called | Open | The current server path no longer logs during STDIO request handling, but the utility itself is not suitable for protocol output streams yet. |
...
Remaining P1 items are documentation or engineering-boundary issues, not hidden fake implementations. They stay visible in docs instead of being presented as fully closed capabilities.
