...
| `integration` | Verify public entrypoints actually close the loop | `tests/integration/mcp_stdio_smoke.ps1` | Public MCP STDIO entrypoint can receive and answer requests | Full MCP protocol coverage or long-running server semantics |
...
| `mcp` | Track MCP separately from CLI and quality | currently covered by `tests/integration/mcp_stdio_smoke.ps1` | Minimal request/response on verified methods | prompts/resources/streaming/HTTP transports |
