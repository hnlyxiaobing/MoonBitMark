...
| **DOCX** | ✅ 完成 | 纯 MoonBit libzip + FFI (expat) |
...
> **注意:** 已实现纯 MoonBit 版本的 libzip，可完全移除此依赖。

- **expat** (>= 2.2) - XML 解析 (DOCX 需要)

如需使用纯 MoonBit 实现，无需安装 libzip。
...
- ⚠️ 格式保留 (粗体/斜体) - 待实现
- ⚠️ 表格支持 - 待实现
- ✅ Pure MoonBit libzip - 已完成 (CRC32 + Deflate)
- ⚠️ 集成测试 - 进行中
